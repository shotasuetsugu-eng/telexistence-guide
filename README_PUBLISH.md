...
Google Drive、Google Map、Slack検索は管理者以外のログイン済みユーザーも利用できます。
未ログイン時にページを開くとログイン案内が表示され、APIは認証なしでは呼び出せません。
...
2. 「OAuth & Permissions」→「Scopes」→「User Token Scopes」に以下を追加:
   - `search:read`（メッセージ検索）
   - `channels:read`（チャンネル一覧）
3. 「Install to Workspace」でインストールし、**User OAuth Token**（`xoxp-...`）を取得
4. 環境変数に設定:
   ```
   SLACK_BOT_TOKEN=xoxp-your-user-token
   SLACK_ALLOWED_CHANNELS=general,install-guide
   ```
   `SLACK_ALLOWED_CHANNELS` には、検索してよいチャンネル名だけをカンマ区切りで入れてください。
